...
export function DenunciarBotao({ alvoTipo, alvoId }: { alvoTipo: "post" | "comment" | "question" | "answer" | "profile"; alvoId: string }) {
  const { user } = useAuth();
  ...
  return (
    <>
      <button
        ...
        className="inline-flex min-h-10 items-center rounded-lg px-2.5 text-xs font-semibold text-muted-foreground"
      >
        <Flag className="size-4" aria-hidden />
      </button>

      {aberto ? (
        <div role="dialog" aria-modal="true" aria-label="Denunciar conteúdo" className="fixed inset-0 z-50 flex items-end bg-foreground/40 p-0">
          <div className="w-full rounded-t-2xl border-t border-border bg-card p-4 pb-8">
            <h2 className="text-base font-bold">Denunciar conteúdo</h2>
            <p className="mt-1 text-xs text-muted-foreground">A moderação analisa todas as denúncias. Sua identidade não é exibida.</p>
            ...
            {estado === "erro" ? <p className="mt-2 text-sm text-destructive">Não foi possível enviar. Tente novamente.</p> : null}

            <div className="mt-3 flex gap-2">
              <button type="button" onClick={() => setAberto(false)} className="min-h-12 flex-1 rounded-xl border border-border text-sm font-semibold">
                Cancelar
              </button>
              <button type="button" onClick={enviar} disabled={estado === "enviando"} className="btn-primary min-h-12 flex-1 justify-center">
                Enviar
              </button>
            </div>
          </div>
